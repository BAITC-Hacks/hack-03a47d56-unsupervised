...
Для версии, описанной в этом README, ожидаются **91 тест и `OK`**. `Ran 0 tests` не является успешной проверкой проекта: обычно команда выполнена не из той папки.
...
| Порт занят, `WinError 10048` или `Address already in use` | Используйте другой порт, например `python main.py serve --port 8766`, и откройте соответствующий адрес |
